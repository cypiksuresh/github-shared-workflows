...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/automation-auto-release.yml@master
...
jobs:
  build-and-upload-to-s3:
    ...
    uses: cypiksuresh/shared-workflows/.github/workflows/automation-build-and-upload-to-s3.yml@master
    with:
      ...
      s3-location: cypiksuresh-ce-shared-artifacts/iam-rolesanywhere-lambdas
...
Enforce assigning labels before merging PR's. Useful for governing the use of semantic versioning labels for [Auto release](https://github.com/cypiksuresh/shared-workflows#auto-release).
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/automation-enforce-release-labels.yml@master
```
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/automation-housekeeping.yml@master
    secrets: inherit
    with:
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/automation-multi-build.yml@master
    secrets: inherit
    with:
      # Required
      image-repo: cypiksureshdk/repo-name
...
Status check that fails if a PR is on-hold, works in conjunction with status_checks [housekeeping](https://github.com/cypiksuresh/shared-workflows#housekeeping)
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/automation-on-hold-prs.yml@master
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/automation-renovate-pr-commenter.yml@master
    with:
...
A Github Action step that runs Checkov against a Terraform plan file. Policies are defined in cypiksuresh/iac-terraform-policies repo.
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/security-gitleaks.yml@master
    secrets: inherit
```
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/security-tfsec-pr-commenter.yml@master
```
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/security-tfsec-upload.yml@master
```
...
jobs:
  shared:
    uses: cypiksuresh/shared-workflows/.github/workflows/security-trivy-iac-check.yaml@master
```
